@call :init  Multi-Windows - latest version at pastebin.com/bBw0Avc4 or git.io/MediaCreationTool.bat

set CHOICES= 1507, 1511, 1607, 1703, 1709, 1803, 1809, 1903 [19H1], 1909 [19H2], 2004 [20H1]
...
:: choices dialog
if not defined MCT_VERSION call :choices MCT_VERSION "%CHOICES%" 11 "Seleziona una versione" 12 white dodgerblue 300
@goto version-%MCT_VERSION%

:version-0
%<%:e1 " Chiusura di MediaCreationTool... "%>% & popd & timeout /t 3 >nul & exit
...
:init script
@echo off & title Multi-Windows by Fabrizio Amorelli - Universal MCT Wrapper by AveYo & color 9f & mode 120,30
:: self-echo top 2-20 lines of script
:: <"%~f0" (set/p \=&for /l %%/ in (1,1,20) do set \=& set/p \=& call echo;%%\%%)
:: lean xp+ color macros by AveYo:  %<%:af " hello "%>>%  &  %<%:cf " w\"or\"ld "%>%    for single \ / " use .%|%\  .%|%/  \"%|%\"
echo Universal MCT Wrapper by AveYo
echo https://git.io/MediaCreationTool.bat
echo Seleziona una versione di Windows 10
echo . . .
...
rem for %%s in (latest_MCT_script.url) do if not exist %%s (echo;[InternetShortcut]&echo;URL=https://git.io/MediaCreationTool.bat)>%%s
:: use MCT workfolder
rem pushd "%~dp0" & mkdir MCT >nul 2>nul & pushd MCT
pushd "%tmp%" & mkdir MCT >nul 2>nul & pushd MCT
...
function Choices ($outputvar,$choices,$sel=1,$caption='Choose',[byte]$sz=12,$bc='MidnightBlue',$fc='Snow',[string]$min='300') {
 ...
 $global:rez=''; $bt=@(); $ch=($choices+',Chiudi').split(','); $ch |foreach {$b=New-Object System.Windows.Forms.Button; $b.Name=$i
